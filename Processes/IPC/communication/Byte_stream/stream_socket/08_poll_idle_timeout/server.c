...
#define TIMEOUT_MS 5000 // 5-second polling timeout

int main()
{

	printf("\n============================================\n");
	printf("||     POLL IDLE TIMEOUT SOCKET SERVER    ||\n");
	printf("============================================\n\n");
	...
	printf("Server listening on port %d (Poll timeout: %d ms)...\n", PORT, TIMEOUT_MS);

	int idle_ticks = 0;

	while(1){

		int poll_ret = poll(fds, (nfds_t)nfds, TIMEOUT_MS);
		...
		if(poll_ret == 0){
			...
			idle_ticks++;
			printf("[HEARTBEAT #%d] Server is idle (No activity across %d descriptors for %d ms)...\n",
			       idle_ticks, nfds, TIMEOUT_MS);
			continue;
			...
			if(fds[i].revents & POLLIN){
				...
				if(fds[i].fd == sockfd){
					...
					if(nfds >= MAX_CONNECT + 1){
						...
						printf("[WARNING] Connection pool full. Dropping client.\n");
						close(client_fd);
						continue;
					...
					printf("\n[CONNECT] New client attached (FD: %d, IP: %s)\n",
					...
					printf("[SERVER] -> Received data from FD %d: \"%s\"\n", fds[i].fd, client_text);
					...
					int resp_len = snprintf(response, sizeof(response),
					                        "Processed by Timeout Server. Word count: %d\n", word_count);
