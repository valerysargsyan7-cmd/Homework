...
int main()
{
	...
	printf("\n============================================\n");
	printf("||      REMOTE LOGGER SOCKET CLIENT       ||\n");
	printf("============================================\n\n");
	...
	printf("Connecting to logger server at %s:%d...\n", IP, PORT);
	...
	printf("[SUCCESS] Connected to Remote Logger Server!\n\n");

	char text[TEXT_SIZE];
	memset(text, 0, sizeof(text));

	printf("Enter log message to transmit: ");
	...
	if(send(sockfd, text, strlen(text) + 1, 0) < 0){
		...
		perror("[ERROR] send log message failed");
		close(sockfd);
		return 1;
	...
	if(bytes_recv < 0){
		...
		perror("[ERROR] recv acknowledgment failed");
		close(sockfd);
		return 1;
